...
<img src="img/capture/평점 등록.png" width="80%"></img>
...
평점의 수정과 등록은 redis의 정보들을 List에 모아서 JdbcTemplate의 batchUpdate()를 이용해 한번에 처리했습니다. batchUpdate()를 이용하는 것이 save(), saveAll(), dirtyChecking을 이용하는 것보다 빨른 결과를 얻었습니다.

<img src="img/capture/평점 등록 테스트.PNG" width="50%"></img>
...
<img src="img/capture/평점 조회.png" width="80%"></img>

'영화에 대한 평점 조회' 와 '개인 평점 조회' 으로 API가 분리되어 있었습니다.

두 API를 합쳐 하나의 요청으로 반환이 가능하도록 하였습니다. 그 이유는 한 페이지에서 해당 영화와 해당 유저에 대한 평점을 같이 보여주기 떄문입니다.

평점 정보를 캐시에서 조회하는데, 캐시에 없다면 캐시를 생성할 수 있도록 평점 등록을 구현할 때 만들었던 메서드를 재활용하였습니다.

처음에는 평점 등록을 할 때 등록된 평점에 대한 정보도 함께 반환하려고 하였으나 읽기와 쓰기가 같은 요청에서 이루어진다면 성능상 문제가 있을 것으로 판단하여 쓰기 요청에 조회 기능을 추가하지 않았습니다.
...
<img src="img/capture/게시글 좋아요.png" width="80%"></img>
...
<img src="img/capture/게시글 상세보기.png" width="80%"></img>
